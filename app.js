...
const elements = {
  ...
  themeToggles: [document.querySelector("#theme-toggle"), document.querySelector("#mobile-theme-toggle")],
};
...
function setStatus(message, variant = "") {
  ...
  elements.statusCard.classList.remove("success", "error");
  if (variant) elements.statusCard.classList.add(variant);
...
function createLogoBurst(x, y) {
  if (isMotionReduced() || !elements.particles) return;
  const amount = window.innerWidth <= 760 ? 11 : 8;

  for (let index = 0; index < amount; index += 1) {
    const particle = document.createElement("span");
    const angle = (Math.PI * 2 * index) / amount + (Math.random() - 0.5) * 0.52;
    const distance = (window.innerWidth <= 760 ? 42 : 34) + Math.random() * 54;
    const size = 10 + Math.random() * 12;

    particle.className = "logo-particle";
    particle.style.setProperty("--touch-x", `${x}px`);
    particle.style.setProperty("--touch-y", `${y}px`);
    particle.style.setProperty("--dx", `${Math.cos(angle) * distance}px`);
    particle.style.setProperty("--dy", `${Math.sin(angle) * distance - 14}px`);
    particle.style.setProperty("--size", `${size}px`);
    particle.style.setProperty("--rotate", `${Math.random() * 60 - 30}deg`);
    particle.style.setProperty("--duration", `${620 + Math.random() * 320}ms`);
...
function setupInteractiveMotion() {
  const tiltTargets = document.querySelectorAll(".wallet-card, .status-card, .card, .profile-pill");

  tiltTargets.forEach((surface) => {
    surface.classList.add("tilt-surface");
    surface.addEventListener("pointermove", (event) => {
      if (isMotionReduced() || event.pointerType === "touch") return;
      ...
      surface.style.setProperty("--lift", "-2px");
    });
    surface.addEventListener("pointerleave", () => {
      surface.style.removeProperty("--tilt-x");
      surface.style.removeProperty("--tilt-y");
      surface.style.removeProperty("--lift");
    });
    surface.addEventListener("pointerdown", (event) => {
      if (isMotionReduced() || event.pointerType !== "touch") return;
      surface.style.setProperty("--tilt-x", "1.3deg");
      surface.style.setProperty("--tilt-y", "-1.1deg");
      surface.style.setProperty("--lift", "-2px");
    });
  ...
  document.addEventListener("pointerdown", (event) => {
    createLogoBurst(event.clientX, event.clientY);
  });
}
...
function setTheme(theme) {
  ...
  try {
    ...
    // Theme remains active for this view if storage is unavailable.
  }
  const themeColor = document.querySelector('meta[name="theme-color"]');
  if (themeColor) themeColor.setAttribute("content", theme === "dark" ? "#070b14" : "#0052ff");
...
function setupTheme() {
  ...
  setTheme(stored || (deviceDark ? "dark" : "light"));
  elements.themeToggles.forEach((toggle) => {
    if (!toggle) return;
...
function setupMobileSettings() {
  if (!elements.settingsToggle || !elements.settingsFields) return;
  elements.settingsToggle.addEventListener("click", () => {
    ...
    elements.settingsFields.classList.toggle("open", !expanded);
  });
}
